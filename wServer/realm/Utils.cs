...
namespace wServer.realm
{
    internal static class EntityUtils
    {
        ...
        public static Player getNearestPlayer(this Entity entity, double Range)
        {
            IEnumerable<Player> nearest = entity.Owner.PlayersCollision.HitTest(entity.X, entity.Y, 16).OfType<Player>();
            if (!nearest.Any()) return null;
            Player closest = nearest.First();
            double dist = Range+1;
            foreach (Player i in entity.Owner.PlayersCollision.HitTest(entity.X, entity.Y, 16).OfType<Player>())
            {
                
                dist = i.Dist(entity);
                if (dist < closest.Dist(entity))
                    closest = i;
            }
            return dist <= Range ?closest:null;
        }
